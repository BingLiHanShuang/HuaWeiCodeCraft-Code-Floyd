...
int main(int argc, char *argv[])
{
	...
	for(int timestamp = 0;timestamp < TIMELIMIT;timestamp++)//carPath_row
	{
		...
		while(isDeadLock)
		{
			Matrix_i2v2 CrossOrder = (Matrix_i2v2)std::malloc(sizeof(int)*crossPath_row*crossPath_row);//记录点到点
			...
			RoadWeight_Changeable(Q, roadMatrix_group, map, crossPath_row, crossPath_row, roadPath_data, crossPath_data);
			...
			Floyd(CrossOrder, crossPath_row, map);
			...
			for(int car_check = 0;car_check < carPath_row;car_check++)//carPath_row
			{
				if(!car_arrive[car_check])
				{
					if(timestamp >= carPath_data[car_check][4])
					{
						...
						{
							//Record
							int startPoint_order = -1;
							for(int search = 0; search < global_crossPath_row; search++)
							{
								if(crossPath_data[search][0] == carPath_data[car_check][1])
								{
									startPoint_order = search;
									break;
								}
							}
							int hope = *(CrossOrder + startPoint_order*crossPath_row + endPoint_order);
							for(int road_check = 1; road_check < 4; road_check++)
							{
								for(int road_check2 = 1; road_check2 < 4; road_check2++)
								{
									if(crossPath_data[carPath_data[car_check][1]][road_check] == crossPath_data[hope][road_check2])
									{
										for(int road_check3 = 1; road_check3 < roadPath_row; road_check3++)
										{
											if(roadPath_data[road_check3][0] == crossPath_data[carPath_data[car_check][1]][road_check])
												next_move[car_check] = road_check3;
										}
									}
								}
							}
